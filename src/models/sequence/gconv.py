...
class GConv(nn.Module):
    ...
    def __init__(
        ...
        for _ in range(self.num_scales):
            if 'randn' in mode:
                ...
                kernel = nn.Parameter(torch.randn(channels, self.h, self.kernel_dim))
                arange = torch.arange(self.l_max) + 1
                hankel = arange.view(1, -1) + arange.view(-1, 1)
                hankel = 2 / (hankel.pow(3) - hankel)
                _, eigvecs = torch.symeig(hankel, eigenvectors=True)
                self.register_buffer('eigvecs', eigvecs[:self.kernel_dim, :])
            else:
            ...
            if 'decay' in mode:
                kernel.data = kernel.data * torch.linspace(1, 2/(decay_max+decay_min), self.kernel_dim)
    ...
    def forward(self, u, state=None, **kwargs): # absorbs return_output and transformer src mask
        ...
        elif 'hankel' in self.mode:
            k = contract('chd, dl -> chl', self.kernel_list[0], self.eigvecs)
        else:
        ...
        if 'expand' in self.mode:
            k = F.interpolate(k, size=self.l_max, mode=interpolate_mode)
        if 'trunc' in self.mode:
            k = F.interpolate(k[...,:self.l_max],size = L,  mode=interpolate_mode)
            k = k * self.l_max/L
